...
ST_COLORMAP: List[List[int]] = [
    ...
    [255, 0, 0],      # 6: sports field
]


_COLORMAP_TO_LABEL_LUT = np.zeros(256 ** 3)
for _i, _cm in enumerate(ST_COLORMAP):
    _COLORMAP_TO_LABEL_LUT[(_cm[0] * 256 + _cm[1]) * 256 + _cm[2]] = _i


def color_label_to_index(color_label: np.ndarray) -> np.ndarray:
    """
    Convert an RGB label image to index map [0..6] using EXACT logic from RS_ST.Color2Index.

    Args:
        color_label: HxWx3 RGB image (uint8 or int-like)

    Returns:
        HxW index map (float64 due to LUT; values in {0..6}). Caller may cast to np.uint8 if needed.
    """
    data = color_label.astype(np.int32)
    idx = (data[:, :, 0] * 256 + data[:, :, 1]) * 256 + data[:, :, 2]
    index_map = _COLORMAP_TO_LABEL_LUT[idx]
    index_map = index_map * (index_map < NUM_CLASSES)
    return index_map


def rgb_labels_to_index_maps(
    label1_dir: str,
    label2_dir: str,
    out_label1_dir: str,
    out_label2_dir: Optional[str] = None,
    overwrite: bool = True,
) -> None:
    """
    Convert all RGB label images in label1_dir and label2_dir to index maps and save to output dirs.
    The mapping and behavior strictly mirror RS_ST.Color2Index.

    Args:
        label1_dir: Directory containing label1 RGB images (e.g., *.png)
        label2_dir: Directory containing label2 RGB images (e.g., *.png)
        out_label1_dir: Output directory to save converted index maps for label1
        out_label2_dir: Output directory to save converted index maps for label2 (defaults to out_label1_dir's sibling if None)
        overwrite: If True, overwrite existing files
    """
    if out_label2_dir is None:
        out_label2_dir = out_label1_dir

    os.makedirs(out_label1_dir, exist_ok=True)
    os.makedirs(out_label2_dir, exist_ok=True)
    ...
    label1_files = _list_images(label1_dir)
    label2_files = _list_images(label2_dir)

    # Process label1
    for name in label1_files:
        src = os.path.join(label1_dir, name)
        dst = os.path.join(out_label1_dir, name)
        if (not overwrite) and os.path.exists(dst):
            continue
        img = io.imread(src)
        index_map = color_label_to_index(img)
        io.imsave(dst, index_map.astype(np.uint8))

    # Process label2
    for name in label2_files:
        src = os.path.join(label2_dir, name)
        dst = os.path.join(out_label2_dir, name)
        if (not overwrite) and os.path.exists(dst):
            continue
        img = io.imread(src)
        index_map = color_label_to_index(img)
...
#     p1 = "/public/home/xjzx14/zx/LISARS_TRAINCLIP/dataset/SECOND/test/label1/00004.png"
#     p2 = "/public/home/xjzx14/zx/LISARS_TRAINCLIP/dataset/SECOND/test/label2/00004.png"

#     img1 = io.imread(p1)
#     img2 = io.imread(p2)

#     label1_idx = color_label_to_index(img1).astype(np.uint8)
#     label2_idx = color_label_to_index(img2).astype(np.uint8)

#     # Print brief info so you can see the two results clearly
#     print("label1_idx:", label1_idx.shape, label1_idx.dtype, int(label1_idx.min()), int(label1_idx.max()))
#     print(label1_idx)
#     print("label2_idx:", label2_idx.shape, label2_idx.dtype, int(label2_idx.min()), int(label2_idx.max()))
#     print(label2_idx)
