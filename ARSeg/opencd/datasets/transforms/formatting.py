...
class MultiImgPackSegInputs(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'gt_seg_map' in results:
            tensor_list = []
            for seg_map in results['gt_seg_map']:
                # 对每个元素执行.astype(np.int64)，然后to_tensor
                tensor = to_tensor(seg_map.astype(np.int64))
                tensor_list.append(tensor)
            # 把tensor_list堆叠成一个tensor
            stacked_tensor = torch.stack(tensor_list, dim=0)
            gt_sem_seg_data = dict(data=stacked_tensor)
