    ...
    def __get_item__(self, i): 
        ...
        if self.has_img:
            ...
            if type(image1_file) == str:
                image1 = self.vis_processor(image1_file) 
            ...
            if type(image2_file) == str:
                image2 = self.vis_processor(image2_file) 
            ...
            if self.has_mask:
                assert isinstance(image1_file, str), "image_file must be a string"
                image1_g = Image.open(image1_file).convert("RGB") 
                image2_g = Image.open(image2_file).convert("RGB") 
                w, h = image1_g.size
                ori_hw = (h, w)
                image1_g = self.vis_processor_gr(image1_g) 
                image2_g = self.vis_processor_gr(image2_g) 
                ...
                sample['image1_g'] = image1_g
                sample['image2_g'] = image2_g
